...
{
    public class SPIGGOWImportViewModel : RecordImporterBase
    {
        public SPIGGOWImportViewModel()
        {
            ReplaceList = true;
        }
        ...
        public override List<PropertyType> AvailibleFields => GetProperties(typeof(SPI_GGOW));
        ...
        public override void SaveClick()
        {
            ...
            if (ReplaceList)
            {
                var records = Database.SPI_GGOWs;
                foreach (var item in records)
                    Database.SPI_GGOWs.Remove(item);
            }

            foreach(DataRow r in ImportDataTable.Rows)
            {
                SPI_GGOW record = new SPI_GGOW();
