...
{
    public class SPISPOWImportViewModel : RecordImporterBase
    {
        public SPISPOWImportViewModel()
        {
            ReplaceList = true;
        }
        ...
        public override List<PropertyType> AvailibleFields => GetProperties(typeof(SPI_SPOW));
        ...
        public override void SaveClick()
        {
            ...
            if (ReplaceList)
            {
                var records = Database.SPI_SPOWs;
                foreach (var item in records)
                    Database.SPI_SPOWs.Remove(item);
            }

            foreach(DataRow r in ImportDataTable.Rows)
            {
                SPI_SPOW record = new SPI_SPOW();
        ...
        public override List<string> RecordTypeSaveCheck()
        {
            ...
            //issues.AddRange(CheckTpes(typeof(SPI_SPOW)));
