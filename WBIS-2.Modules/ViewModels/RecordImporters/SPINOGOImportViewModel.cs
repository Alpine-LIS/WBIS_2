...
{
    public class SPINOGOImportViewModel : RecordImporterBase
    {
        public SPINOGOImportViewModel()
        {
            ReplaceList = true;
        }
        ...
        public override List<PropertyType> AvailibleFields => GetProperties(typeof(SPI_NOGO));
        ...
        public override void SaveClick()
        {
            ...
            if (ReplaceList)
            {
                var records = Database.SPI_NOGOs;
                foreach (var item in records)
                    Database.SPI_NOGOs.Remove(item);
            }

            foreach(DataRow r in ImportDataTable.Rows)
            {
                SPI_NOGO record = new SPI_NOGO();
        ...
        public override List<string> RecordTypeSaveCheck()
        {
            ...
            //issues.AddRange(CheckTpes(typeof(SPI_NOGO)));
